 ...
 * @LastEditTime: 2024-04-09 20:54:00
 * @Description: 描述
 */
import { create } from '@/common/services';
import { useCRUDService } from '@/hooks';

export const useAppUsageRecord = <T extends AppUsageRecordType>(options: Partial<CRUDActionType<T>> = {}) => {
  ...
  return {
    async listHandler(params: PageParamsType & unknown) {
      return await postListHandler(`/behavior/listAllUserBehavior`, { params: params });
    },
    async detailHandler() {
      if (!currentRecord?.id) {
        return {} as T;
